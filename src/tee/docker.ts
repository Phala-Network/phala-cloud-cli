...
import { COMPOSE_FILES_DIR, COMPOSE_TEMPLATES, ComposeTemplateSchema, ComposeTemplate } from './constants';
...
export class DockerOperations {
  ...
  async buildComposeFile(
    ...
    const template = COMPOSE_TEMPLATES[templateId];
    if (!template) {
      throw new Error(`Template "${templateId}" not found. Available templates: ${Object.keys(COMPOSE_TEMPLATES).join(', ')}`);
    }
    ...
    const composeFile = path.join(composePath, `tee-compose.yaml`);
  ...
  async runLocal(tag: string, envFile: string, version: string = 'basic'): Promise<void> {
    try {
      const composeFile = await this.buildComposeFile(tag, envFile, version);
  ...
  // Helper method to list available templates
  static listTemplates(): ComposeTemplate[] {
    return Object.entries(COMPOSE_TEMPLATES).map(([id, template]) => ({
      ...ComposeTemplateSchema.parse(template),
      id
    }));
  }
}